...
export default function App() {
    ...
    (async () => {
      ...
      if (alive && s) {
        ...
        bs.forEach((b) => {
          ...
          const el = (b.status === 'running' ? b.accMs + (t - b.startTs)
            : b.accMs) / 1000;
          firedRef.current[b.id] = new Set(sch.events
            .filter((e) => e.at <= el).map((e) => e.id));
        });
        setBatches(bs);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (doneIds.length) setBatches((prev) => prev.map((b) =>
        doneIds.includes(b.id) ? { ...b, status: 'done' } : b));
    }, 250);
  ...
  const summary = useMemo(() => {
    ...
    return best;
  }, [batches, now]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const toggleStage = (id) => setStaged((s) =>
  ...
  const pauseLeva = (id) => setBatches((p) => p.map((b) =>
    b.id === id && b.status === 'running' ?
      { ...b, status: 'paused', accMs: b.accMs + (Date.now() - b.startTs), startTs: null } : b));

  const resumeLeva = (id) => {
    ensure();
    setBatches((p) => p.map((b) =>
      b.id === id && b.status === 'paused' ?
        { ...b, status: 'running', startTs: Date.now() } : b));
  };
